<script lang="ts">
	...
	import { ButtonId, SliderId, SliderStateType, WHEEL_COUNT_PER_ROTATION } from './constants';
	...
	function extractElements(labelPrefix: string): Record<string, HTMLElement> {
		const els = Array.from(
			svgElement.querySelectorAll<HTMLElement>(`[inkscape\\:label^=${labelPrefix}]`)
		);
		return Object.fromEntries(
			els.map((el) => {
				const labelSuffix = el.getAttribute('inkscape:label').substring(labelPrefix.length);
	...
	$: ({ inner: joystickInner } = svgElement ? extractElements('joystick-') : {});
	...
	$: ledIcons.forEach((el, i) =>
		ledStates[i] ? el.classList.add('led-active') : el.classList.remove('led-active')
	);
	...
	$: if (sliderHandle) {
		sliderHandle.style.setProperty('transform', `translateX(${((slider - 64) / 64) * 130}%)`);
		debounce(sliderHandle, 'slider');
	}

	onMount(() => () => {
		sliderHandle?.style.removeProperty('transform');
		sliderHandle?.classList.remove('slider-active');
		joystickInner?.style.removeProperty('transform');
		joystickInner?.classList.remove('slider-active');
		Object.values(wheelElements ?? {}).forEach((el) => {
			el.style.removeProperty('tranform');
			el.classList.remove('slider-active');
		});
		Object.values(buttonElements ?? {}).forEach((el) => el.classList.remove('active-button'));
		ledIcons.forEach((el) => el.classList.remove('led-active'));
	});
</script>
